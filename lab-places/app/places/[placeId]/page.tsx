...
export default async function PlacePage({ params }: Props) {
  ...
  /* ── Shared header ───────────────────────────────────────────────── */
  const Header = (
    <header className="relative z-10 flex-shrink-0 px-4 md:px-8 pt-3.5 pb-3 border-b border-white/[0.06]">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1.5 mb-2">
        <Link
          href="/"
          className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-[0.14em] text-white/35 hover:text-white/60 transition-colors"
        >
          <ArrowLeft size={9} />
          LAB Places
        </Link>
        <span className="text-white/20 text-[9px]">/</span>
        <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-white/50">{place.shortLabel}</span>
      </nav>
      {/* Title + status */}
      <div className="flex items-center gap-2.5">
        <h1 className="text-[1.45rem] font-black text-white tracking-tight leading-none">{place.shortLabel}</h1>
        <StatusChip status={place.status} />
      </div>
      <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-white/30 mt-1">{place.descriptor}</p>
    </header>
  );
  ...
  return (
    <>
      {/* ══════════════════════════════════════════════════════════════
          MOBILE — h-screen with internal scroll → no native scrollbar
          ══════════════════════════════════════════════════════════════ */}
      <div className="md:hidden h-screen bg-[#0e0e0e] overflow-hidden flex flex-col">
        ...
        {Header}
        <div className="relative z-10 flex-1 overflow-y-auto scrollbar-none px-3.5 pt-2.5 pb-4 flex flex-col gap-2">

          {/* 1. Color banner */}
          <div
            className="flex-shrink-0 h-44 rounded-2xl overflow-hidden relative flex flex-col justify-between"
            style={{
              background: [
          ...
          >
            ...
            <div className="flex items-start justify-between px-4 pt-4">
              ...
              {place.attention && (
                <div className="flex items-center gap-1 ml-2">
                  ...
                  <p className="text-[9px] font-semibold text-amber-300 truncate max-w-[110px]">{place.attention.title}</p>
                </div>
              )}
            </div>
            <div className="px-4 pb-4">
              ...
              <p className="text-[11px] text-white/48 mt-1 leading-snug max-w-[210px]">
                {place.shortSummary}
              </p>
            </div>
          </div>
          ...
          {/* 4. Full action rail — single column */}
          <div className="flex-shrink-0">
      ...
      {/* ══════════════════════════════════════════════════════════════
          DESKTOP — h-screen fixed, inspect-rich 3-col
          ══════════════════════════════════════════════════════════════ */}
